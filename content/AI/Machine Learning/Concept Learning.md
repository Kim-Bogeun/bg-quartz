...
이걸 조금만 수학적으로 표현하자면… 이 고양이라는 개체로 분류할 수 있는 함수가 $c$ 로 존재하고, 이와 유사하게 동작할 거라 기대하는 가설 함수 $h$가 있다. 그리고 이 $h$를 조정하는 $x$들로 울음소리, 다리, 털 등의 input이 있는 것이다.

즉 이 Concept은 하나의 목표 함수(function)로 볼 수 있고, Concept Learning은 데이터를 기반으로 다양한 가설 $h$를 검증, 수정해나가며 사물을 **분류하는 함수** $c$**를 추론**하는 과정이다.
...
이 Concept를 추정하기 위한 여러 알고리즘이 책에 소개되어 있지만, 다음에 등장할 [[Inductive Bias]]의 공부에 크게 상관이 있는 파트는 아니라고 판단, 우선은 넘어가기로..
